import { supabase } from '../lib/supabase.ts'
  import { applyAutoSubsAndMultipliers, bonusFromBpsRows, fetchFixtures, fetchLiveGW, getPlayerLiveComputed, getTeamGwStatus, type FixtureInfo, type LiveStatLine, type TeamStatus } from '../lib/fplLive.ts'
  ...
  const LIVE_CACHES_TTL_MS = 60_000; // 60s – adjust or remove TTL if you prefer
...
function getFixtureProjectedBonus(fixture: FixtureInfo, playerId: number): number {
  ...
  const rows = [...h, ...a]
    ...
    .filter(row => Number.isFinite(row.element) && Number.isFinite(row.value));

  return bonusFromBpsRows(rows).get(playerId) || 0;
}
...
function buildBreakdownRows(
  ...
  function renderBpsCell(entry?: BpsEntry) {
    if (!entry) {
      return `<span class="bps-empty">—</span>`;
    }
  ...
  async function showPlayerModal(playerId: number) {
    ...
    for (const fx of fixtureBlocks) {
      ...
      const statusText = isFinished ? "Full Time" : (started ? "🔴 Live" : "Not Started");
      ...
      const fixtureKey = makeFixtureKey(Number(fx.event), Number(oppTeamId), isHome);
      ...
      const fixtureProjBonus = getFixtureProjectedBonus(fx, playerId);

      const breakdown = buildBreakdownRows(
        fixtureStats,
        player.position,
        isFinished,
        explainStatsRows.length ? 0 : fixtureProjBonus
      );
      ...
      if (!isFinished && !explainStatsRows.length && fixtureProjBonus > 0) {
        html += `
  ...
  async function showSquadModal(entryId: number, managerName: string, teamName: string, chipPlayed: string, livePoints: number) {
    ...
    try {
      ...
      for (const p of originalStarters) {
        ...
        const ptsText = effMult > 1 ? `${p.liveTotal}×${effMult}` : String(displayPts);
      ...
      for (const p of subbedInPlayers) {
        ...
        const ptsText = mult > 1 ? `${p.liveTotal}×${mult}` : String(displayPts);
  ...
  function makeLiveTable(managers: any[], compareToId = "") {
    ...
    for (const m of sorted) {
      ...
      const diffText = diffOverall > 0 ? `+${diffOverall}` : diffOverall === 0 ? "—" : `${diffOverall}`;
  ...
  async function loadLive(ids: number[], gw: number, opts?: { force?: boolean; background?: boolean }) {
    ...
    loadingStatus.innerHTML = `<span class='text-success font-bold'>✅ Loaded ${liveData.length} managers</span>`;
  ...
  async function init() {
    ...
    if (isSnapshotUsable(snap, gw, managerIds)) {
      ...
      loadingStatus.innerHTML = `<span class='text-success font-bold'>✅ Loaded ${liveData.length} managers (cached)</span>`;
      return;
