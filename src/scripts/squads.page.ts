 import { supabase } from '../lib/supabase.ts'
      ...
      html += `
        <div class="card squad-card">
          ...
          <div class="squad-header">
            ...
            <span class="team-name-span">— ${manager.teamName}</span>
          </div>
  ...
  async function showManagerHistoryModal(managerId) {
    ...
    // Ensure bootstrap caches exist when page was restored from snapshot
    if (playerCache.size === 0 || teamCache.size === 0) {
  ...
  async function showPlayerModal(playerId) {
    let player = playerCache.get(playerId);

    if (!player && (playerCache.size === 0 || teamCache.size === 0)) {
      const snap = loadSquadsSnapshot();
      const cacheBuster = snap?.savedAt ? String(snap.savedAt) : undefined;
      await fetchBootstrapData(cacheBuster);
      player = playerCache.get(playerId);
    }
      ...
      <div class="player-modal-breakdown">
        ...
        <div class="breakdown-list">
          <div class="breakdown-row">
            ...
            <span class="breakdown-points">—</span>
          </div>
          <div class="breakdown-row">
  ...
  async function loadSquads(ids, gw, opts?: { force?: boolean }) {
   ...
   loadingStatus.textContent = `Fetching latest data...`;
    ...
    loadingStatus.innerHTML = `<span class="text-success font-bold">✅ Loaded ${allSquadsData.length} managers for GW${gw}</span>`;
    if (lastUpdatedEl) {
      lastUpdatedEl.textContent = formatUpdateTime(Date.now());
    }
  ...
  async function init() {
    ...
    if (
      ...
      // Ensure bootstrap caches exist when page was restored from snapshot
      const cacheBuster = String(snap.savedAt || Date.now());
      if (playerCache.size === 0 || teamCache.size === 0) {
        await fetchBootstrapData(cacheBuster);
      }
      ...
      loadingStatus.textContent = "";
      if (lastUpdatedEl) {
        lastUpdatedEl.textContent = formatUpdateTime(snap.savedAt);
      }
      return;
    }
  ...
  async function updateDisplay(opts?: { force?: boolean }) {
    const force = !!opts?.force;
    managerCountEl.textContent = `${managerIds.length} managers`;
    if (managerIds.length === 0) {
      out.innerHTML = "<p class='status status--error'>No manager IDs configured. <a href='/managers'>Add managers here</a>.</p>";
    ...
    if (managerParam) {
      ...
      await loadSquads([managerId], gw, { force });
      ...
      await loadSquads(managerIds, gw, { force });
    }
  }
  ...
  refreshBtn.addEventListener("click", async () => {
    ...
    await updateDisplay({ force: true });
  });
